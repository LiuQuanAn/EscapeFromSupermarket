 ...
 * LOCAL FORK NOTICE
 *
 * Forked from upstream master (commit b8a1ba4815764df747f7fbc54f6847dbd34f5f2a)
 * on 2026-05-23. Upstream tracking intentionally abandoned — this file is the
 * canonical version for project escape-from-supermarket.
 *
 * Divergences from upstream:
 *   - Architecture<T>: type-keyed pending init (Dictionary, not HashSet);
 *     nested-registration-safe drain loop; thread-safe MakeSureArchitecture
 *     via lock; added static Reset() for scene reload / test isolation.
 *   - BindableProperty<T>: instance-level Comparer (was static, polluted
 *     across instances); default EqualityComparer<T>.Default.Equals;
 *     Value setter null-safe; ToString null-safe.
 *   - CustomUnRegister: idempotent UnRegister (null-coalesced invoke).
 *   - BindablePropertyUnRegister: idempotent UnRegister.
 *   - OrEvent.UnRegister: only cascades source unsubscribe when no
 *     listeners remain.
 *   - UnRegisterWhenNodeExitTree: returns wrapper that detaches the Godot
...
namespace QFramework
{
	...
	public abstract class Architecture<T> : IArchitecture where T : Architecture<T>, new()
	{
		...
		private Dictionary<Type, IModel> mModels = new Dictionary<Type, IModel>();
		...
		static void MakeSureArchitecture()
		{
			lock (mInitLock)
			{
				...
				while (arch.mModels.Count > 0)
				{
					var batch = new List<IModel>(arch.mModels.Values);
					arch.mModels.Clear();
					foreach (var m in batch) m.Init();
				}

				while (arch.mSystems.Count > 0)
				{
					var batch = new List<ISystem>(arch.mSystems.Values);
					arch.mSystems.Clear();
					foreach (var s in batch) s.Init();
				}
		...
		public void RegisterSystem<TSystem>(TSystem system) where TSystem : ISystem
		{
			...
			if (!mInited)
			{
				mSystems[typeof(TSystem)] = system;
			}
			else
			{
				system.Init();
	...
	{
		private class NodeExitUnRegister : IUnRegister
		{
			private IUnRegister mInner;
			private Godot.Node mNode;
			private Action mHandler;

			public NodeExitUnRegister(IUnRegister inner, Godot.Node node)
			{
				mInner = inner;
				mNode = node;
				mHandler = UnRegister;
				node.TreeExiting += mHandler;
			}

			public void UnRegister()
			{
				if (mInner == null) return;
				mNode.TreeExiting -= mHandler;
				mInner.UnRegister();
				mInner = null;
				mNode = null;
				mHandler = null;
		...
		public static IUnRegister UnRegisterWhenNodeExitTree(this IUnRegister unRegister, Godot.Node node) =>
			new NodeExitUnRegister(unRegister, node);
	}
	...
	public class BindableProperty<T> : IBindableProperty<T>
	{
		...
		public T Value
		{
			...
			set
			{
				...
				if (value != null && mValue != null && Comparer(value, mValue)) return;
	...
	public class EasyEvents
	{
		...
		{
			lock (mLock) mTypeEvents[typeof(T)] = new T();
		}

		public T GetEvent<T>() where T : IEasyEvent
		{
			lock (mLock) return mTypeEvents.TryGetValue(typeof(T), out var e) ? (T)e : default;
		}
		...
		public T GetOrAddEvent<T>() where T : IEasyEvent, new()
		{
			lock (mLock)
			{
				if (mTypeEvents.TryGetValue(typeof(T), out var e)) return (T)e;
				var t = new T();
				mTypeEvents[typeof(T)] = t;
				return t;
			}
		}
	}
	...
	public class OrEvent : IUnRegisterList
	{
		...
		{
			mOnEvent -= onEvent;
			if (mOnEvent == null) this.UnRegisterAll();
		}
